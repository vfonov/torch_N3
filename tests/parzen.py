...
iteration, not by anything about the measurement.  ``--parzen-sigma`` replaces
it with the estimator the name promises: a Gaussian of a stated width,
normalised per sample, reaching as many bins as it needs
(:func:`torch_n3.blocks.histogram.histogram`).  Everything downstream is
untouched.
...
should show up as under-sharpening, and the question is where between the two
the useful width sits -- if there is one.
...
2. The shipped protocol end to end on ``brain.mnc``: how many iterations the
   ...
   lands from the linear split's own answer.  The second number says whether
   the window changed the result at all; the first says whether it changed it
   for the better, as far as a single reference volume can.

The sweep is ``len(SIGMAS)`` times ``tables.py``'s work, about a minute per
window on a GPU.  Only ``normal`` is swept: the question here is about the
histogram, and sweeping solvers as well would answer neither question
faster.

**Measured 2026-08-02**, and the ``None`` rows reproduce the published tables
cell for cell, which is the check that nothing else moved.  Residual
non-uniformity at the *shipped* ``--lambda 1e-7``, 20% planted:
...
as harm: it is the best window at 50 mm and the worst at 200 mm/``1e-6``
(1.21x the linear split) and at every ``1e-4`` cell.

**This measurement is noiseless, and that is its blind spot.** One analytic
field on ``brain_nu_ref.mnc``, no noise added.  ``experiments/`` asks the same
question of 450 random fields per window with Gaussian noise at a stated SNR,
and the window's gain there tracks the *noise* rather than the field: at 20%
planted and SNR 20 under the shipped protocol the linear split leaves 4.63%
against ``sigma 4``'s 1.71%, where noiseless they are within a few percent of
each other.  Read ``experiments/README.md``, "The histogram kernel", before
concluding anything from the tables above about a real volume.

A width of 2 bins is 0.052 log units here against ``--fwhm 0.15``'s sigma of
0.064, so the useful window is just under the blur the deconvolution is
already removing, and ``sigma 4`` exceeds it.  That is consistent with the
mechanism -- the window adds a Gaussian that ``sharpen_hist`` was not told
about, so a wide one is under-sharpening -- but it is one volume and one
synthetic field, and nothing here tests the obvious next step, which is to
take the added width back out of ``--fwhm`` in quadrature.

End to end on ``brain.mnc`` under the shipped protocol, for scale: the window
changes the corrected volume by 6.2e-3 relative RMS at ``sigma 0.5`` and
...
def _bin_width():
    ...
    Printed so that a window in bins can be read against ``--fwhm``, which is
    in log units: the two Gaussians -- the one the window adds and the one the
    deconvolution removes -- are otherwise quoted in different currencies.
    """
...
def _against_linear(cells, sigmas, amplitude, lam):
    ...
    The sweep's own comparison: every cell divided by the same cell under N3's
    kernel, so that below 1.00 means the window helped there.
    """
...
def _best(cells, sigmas, amplitude):
    """The best window and weight in each column -- what the sweep is for."""
