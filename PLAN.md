...
| 13 | end-to-end **properties**, asserted before any bound is measured: the field is strictly positive; the in-mask coefficient of variation of the output is below the input's; a volume with no planted non-uniformity yields a **bounded, finite** field; iteration counts match the Perl exactly at `-stop 0`. The "within 1e-3 of constant" wording is dropped: measured on a no-bias two-tissue phantom with additive noise, neither this port nor the legacy reaches it (port field CV ~0.05, legacy ~0.0064). The gap is a recorded over-correction to chase in cycle 14, not a pass criterion | the Perl | properties, not bounds — none of these needs the code to have been run first |
