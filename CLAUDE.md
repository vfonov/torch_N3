...
| `torch_n3/blocks/denoise.py` | Not a port and not part of N3: an optional non-local-means prefilter (`--denoise`), copied from a separate PyTorch reimplementation of Manjon et al. 2010 (`torch_SR/`, studied in `research.md`) so that this package stays self-contained. Off by default, no oracle, and the legacy backend answers it with a refusal. It filters the volume at full resolution *before* `-shrink` — `Volume.shrink` is nearest-neighbour and would otherwise alias the noise away — and feeds the **estimation only**: `nu_evaluate` still divides the original intensities, so the output is corrected and never denoised. Two things to know before touching it. Its noise floor is the one quantity in the filter that carries the intensity scale, so the floor is brought onto the volume's scale rather than the volume onto the floor's — measured against the **dynamic range** (1st-to-99th centile), not the maximum the source uses, which is neither robust nor translation-invariant (`PROBLEMS.md` §1). With the source's fixed `1e-10` variance clamp likewise replaced, the filter is affine-equivariant, which is asserted exactly. And it is **not the same estimator as `torch_SR`'s C kernel**, which uses the first voxel's sigma for both directions of a symmetric pair where this uses each centre's own: they agree to 2.3e-13 at constant sigma and differ 1.0% RMS at per-voxel sigma, so the C code is no oracle for it. On one analytic field it appeared to help less than `--parzen-sigma`; over 450 random fields per cell it does not — it improves 95% of trials at SNR 20 and 88% at SNR 40 while costing 2-6% at SNR infinity, and it makes both N3 and `hoyer` nearly indifferent to noise. The single field also said it was a *substitute* for `--parzen-sigma`; crossed over the same 450 trials the two are **complements** — the pair beats the denoiser alone in all nine cells on the median paired difference and the window alone in all six noisy ones. **Do not quote the single-field table as a verdict**; that is the whole reason `experiments/` exists (`experiments/README.md`, "Prefiltering the volume"). It costs some forty times the estimation it feeds, so it is off by default and belongs on noisy data. |
| `torch_n3/denoise_cli.py` | Standalone entry point around `blocks/denoise.py`: `python3 -m torch_n3.denoise_cli input.mnc output.mnc` loads a MINC volume, filters it, and writes the result — unlike `cli.py`'s `--denoise`, which filters only the copy that feeds the field estimate and writes the caller's original intensities back out. `--search`/`--patch`/`--strength` default from the same `pipeline.DEFAULTS` entries `cli.py` uses. No `--backend`: there is one implementation. |
| `tests/margins.py`, `tests/convergence.py`, `tests/tables.py`, `tests/parzen.py`, `tests/denoise.py` | None is a test; all five measure and print. `python3 -m tests.margins` prints `PROBLEMS.md`'s table — where every comparison sits against its bound. `python3 -m tests.convergence` prints where the two backends stop agreeing as iterations grow, which is a property of the machine's LAPACK rather than of this code; run it on any new platform before trusting an end-to-end number there. `python3 -m tests.tables` re-measures the `--lambda` × `--distance` tables under each direct solver and diffs them against the copies published in `README.md` and `cli.py`. `python3 -m tests.parzen` measures the first of the two *modifications* to the algorithm carried by the port: `--parzen-sigma`, a Gaussian Parzen window in place of N3's linear split (`blocks/histogram.py`). It runs `tables.py`'s sweep once per window at `solver="normal"`, plus the shipped protocol end to end. Its `None` rows are `tables.py`'s published cells, so a mismatch there indicates a change elsewhere. `python3 -m tests.denoise` measures the second, `--denoise` (`blocks/denoise.py`), and **must be read beside `parzen.py`**: both suppress noise-driven variance in the same data term, so the two are candidate substitutes and measuring either alone answers a question nobody asked. It therefore crosses `--denoise` with `--parzen-sigma` over SNR, and — unlike `parzen.py`'s sweep — plants noise, since a noiseless sweep cannot show a spatial filter in a good light and must not be quoted as though it could. Its `off`/`linear`/`snr inf` cell is `tables.py`'s published cell. |
...
- **A threshold on an intensity is a threshold on somebody's choice of scale.** MINC files
  carry whatever range their writer picked — `brain.mnc` peaks at 1,078,824 — so an absolute
  constant compared against a voxel value means nothing until the scale is pinned. This is the
  same class of trap as the float32 one above, and it is silent in both directions:
  `blocks/denoise.py`'s noise floor admits 98.3% of voxels (air included) if the raw
  intensities are used, and 0% on the same anatomy stored on [0, 1]. Everything else in that
  filter is a ratio and therefore scale-free, so exactly one constant needed converting — and
...
- **Pin such a scale to a *range*, never to a level, and never to `max()`.** A threshold on
  ...
  of them: since an outlier can only raise it, and a raised floor *excludes* voxels, a single
  spike degrades a filter towards doing nothing with no diagnostic — measured, one planted
  voxel at 100× took `blocks/denoise.py` from filtering 92.7% of `brain.mnc` to 0%, silently.
  It now uses the 1st-to-99th centile separation, which makes it affine-equivariant. Use
  `kthvalue`, not `torch.quantile`: the latter refuses tensors above 2²⁴ elements (a 512³
  volume exceeds that eightfold), and a *selected* order statistic rescales exactly where an
  interpolated one does not, so the equivariance can be asserted with `torch.equal`.
